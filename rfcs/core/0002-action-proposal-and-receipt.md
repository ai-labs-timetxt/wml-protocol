# RFC 0002: Action Proposal and Receipt

Status: Draft  
Created: 2026-05-01

## Abstract

This RFC defines two central WML concepts:

- `action_proposal`
- `receipt`

An agent should propose actions. It should not directly claim that the world changed.

A receipt is the evidence that an action was executed, inspected, rejected, or deferred.

## Action proposal

An action proposal is a structured request by an agent to use a capability.

Example:

```json
{
  "type": "action_proposal",
  "world_id": "incident-investigation-001",
  "goal_id": "identify_root_cause",
  "target_entity": "service_endpoint",
  "capability_id": "inspect_service_configuration",
  "permission_class": "read_only_auto",
  "arguments": {
    "endpoint_id": "example-endpoint",
    "time_range": "last_30_minutes"
  }
}
```

## Required fields

An action proposal should include:

- `type`
- `world_id`
- `goal_id`
- `target_entity`
- `capability_id`
- `permission_class`
- `arguments`

## Receipt

A receipt records what happened after an action proposal.

Example:

```json
{
  "type": "receipt",
  "world_id": "incident-investigation-001",
  "action_proposal_id": "ap_001",
  "status": "success",
  "permission_class": "read_only_auto",
  "evidence_id": "ev_service_config_001",
  "summary": "Service configuration was retrieved successfully.",
  "timestamp": "2026-05-01T00:00:00Z"
}
```

## Receipt status

Suggested statuses:

- `success`
- `failed`
- `rejected`
- `pending_user_confirmation`
- `skipped`
- `not_applicable`

## Important rule

An agent must not claim success without a receipt or evidence reference.

Bad:

```text
I checked the configuration and found the issue.
```

Good:

```text
Receipt r_001 confirms that the configuration was inspected.
Evidence ev_001 shows the observed setting that supports the conclusion.
```

## Side-effecting actions

Side-effecting actions must not be auto-executed unless the active permission policy explicitly allows them.

Examples of side-effecting actions:

- changing a remote configuration
- writing to a shared datastore
- deleting, replacing, or restarting a resource
- sending a message or notification
- triggering an irreversible workflow

For these actions, an agent may create an action proposal but must mark it as requiring explicit human approval.
